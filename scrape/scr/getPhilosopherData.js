...
const philLists = require('./dataLists/philosophersList'); // modify the file name as needed
const nationalities = natLists.nationalities;
const regions = natLists.regions;//         // vv swap out the names here as needed
const urlArray = philLists.arrayOfUrls; //  // ^^
const indicesSet = philLists.indices;
const arrayOfNames = philLists.arrayOfNames;

const done = chalk.bold.red; // logs when a request has been made
const newPhil = chalk.bold.blue; // logs when a new url has been discovered;

let num;
let indices = indicesSet[num];
  ...
  for (let i = 0; i < indices.length; i++) {
    ...
    request(url, (err, res, html) => {
      if (err) console.log(err);
        ...
        if (!arrayOfNames.includes(phil.name)) {
          console.log(newPhil(indices[i]));
          AllData.create(phil);
        }
